...
const CartProvider = ({ defaultValue = [], children }) => {
    ...
    const addItem = (item, quantity) => {
        ...
        const result = cart.filter(prod => prod.item.id === item.id);

        //Si existe el producto
        if (result.length > 0) {
            //console.log('el producto ya existe!'); 
            const cartProd = cart.map(prod => prod.item.id === item.id ? { ...prod, quantity: quantity + prod.quantity } : prod);
            setCart(cartProd);
        }
        else {
            //Nuevo producto
    ...
    const removeItem = (itemId) => {       
        ...
        const filterProd = cart.filter(prod => prod.item.id !== itemId);
        setCart(filterProd);
    ...
    const isInCart = (id) => {
        ...
        const result = cart.filter(prod => prod.item.id === id);
        if (result.length > 0) {
            return true;
        }
        else {
